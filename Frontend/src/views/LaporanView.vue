    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div class="lg:col-span-8 space-y-5">
        <div class="bg-white dark:bg-[#141d30] rounded-2xl border-2 border-slate-100 dark:border-slate-800/50 shadow-sm p-6 space-y-6">
          ...
          <form @submit.prevent="generateReport" class="space-y-5 text-xs">
            ...
            <div>
              ...
              <div class="relative">
                ...
                >
                  <option :value="null">-- SEMUA PERANGKAT DAERAH (KOTA TEGAL) --</option>
                  <option v-for="opd in opdOptions" :key="opd.id_sub_pd" :value="opd.id_sub_pd">
                    [{{ opd.kode }}] {{ opd.nama_pd }}
                  </option>
                </select>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                >
                  <input type="radio" v-model="filter.format" value="web" class="sr-only" />
                  <Globe class="w-5 h-5" />
                  <span class="font-black text-xs">Web (HTML)</span>
                </label>
                ...
                <label 
                  ...
                  :class="filter.format === 'pdf' 
                    ...
                    : 'border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/40'"
                >
                  <input type="radio" v-model="filter.format" value="pdf" class="sr-only" />
                ...
                <label 
                  class="p-3 rounded-xl border-2 cursor-pointer flex flex-col items-center justify-center transition-all"
                  :class="filter.format === 'excel' 
                    ? 'border-emerald-500 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 shadow-sm' 
                    : 'border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/40'"
                >
                  <input type="radio" v-model="filter.format" value="excel" class="sr-only" />
                ...
                <label 
                  class="p-3 rounded-xl border-2 cursor-pointer flex flex-col items-center justify-center transition-all"
                  :class="filter.format === 'word' 
                    ? 'border-blue-500 bg-blue-500/10 text-blue-600 dark:text-blue-400 shadow-sm' 
                    : 'border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/40'"
                >
                  <input type="radio" v-model="filter.format" value="word" class="sr-only" />
            ...
            <div class="pt-4 border-t-2 border-slate-100 dark:border-slate-800/50 flex flex-col sm:flex-row items-center justify-end gap-3">
              ...
              >
                ...
                <Printer v-else-if="filter.format === 'web' || filter.format === 'pdf'" class="w-4 h-4" />
                <Download v-else class="w-4 h-4" />
                <span>{{ generating ? 'Memproses Laporan...' : getActionButtonText() }}</span>
              </button>
            </div>
...
const rkoItems = [
  ...
  { id: 'rko_1_3', category: 'rko', categoryName: 'RKO', label: 'RKO Bagian I-III (Visi Misi, Alokasi Anggaran, Struktur Organisasi)' },
...
const filter = ref({
  ...
  tahun: 2026,
  bulan: 0,
  footer_text: 'Kota Tegal, 2026 • Dicetak melalui SIMLABA',
  format: 'web'
})
...
const getActionButtonText = () => {
  switch (filter.value.format) {
    case 'web': return 'Tampilkan Laporan (Web)'
    case 'pdf': return 'Cetak Dokumen PDF'
    case 'excel': return 'Unduh File Excel'
    case 'word': return 'Unduh File Word'
    default: return 'Proses Laporan'
  }
}
